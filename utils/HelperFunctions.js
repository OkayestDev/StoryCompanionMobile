    ...
    for (const key in requestData) {
        if (requestData[key] instanceof Array) {
            ...
            for (let attachment in requestData[key]) {
                if (typeof requestData[key][attachment].path === 'string') {
                    formData.append(
                        `${key}[]`,
                        dataURLtoFile(
                            requestData[key][attachment].path,
                            requestData[key][attachment].name
                        ),
                        requestData[key][attachment].name
                    );
                } else {
            ...
            if (requestData[key].path !== null && requestData[key].path !== '') {
                formData.append(
                    `${key}[]`,
                    dataURLtoFile(requestData[key].path, requestData[key].name),
...
export function createQueryString(paramsObject, route) {
    ...
    const signature = `${API_CONFIG.api_secret +
        API_CONFIG.url_suffix +
        route}?${createQueryParamsString(tempParamsObject)}`;
    ...
    if (LOGS.ENABLE_LOGS) {
        console.info(
            `${API_CONFIG.url + API_CONFIG.url_suffix + route}?${createQueryParamsString(
                tempParamsObject
            )}`
        );
    }
    return `${API_CONFIG.url + API_CONFIG.url_suffix + route}?${createQueryParamsString(
        tempParamsObject
    )}`;
}
